...
final class VertexMiddleware implements Middleware
{
    ...
    private function rewriteRequest(RequestInterface $request): RequestInterface
    {
        ...
        if (str_ends_with($path, '/v1/messages/count_tokens')) {
            ...
            return (new RequestTransformer($request, $this->streamFactory))
        ...
        if (false !== ($i = strpos($path, '/v1/'))) {
            throw new AnthropicException(substr($path, $i).' is not supported on the Vertex client.');
        }
    ...
    {
        return sprintf('/v1/projects/%s/locations/%s/publishers/anthropic/models', ($this->projectId)(), $this->location);
    }
}
